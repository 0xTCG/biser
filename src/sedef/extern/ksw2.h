...
static inline uint32_t *ksw_push_cigar(void *km, int64_t *n_cigar, int64_t *m_cigar, uint32_t *cigar, uint32_t op, int len)
{
	if (*n_cigar == 0  || op != (cigar[(*n_cigar) - 1]&0xf)) {
		if (*n_cigar == *m_cigar) {
			if (*m_cigar > 2*1024LL*1024LL*1024LL) *m_cigar = *m_cigar + 1024LL*1024LL*1024LL;
			else *m_cigar = *m_cigar? (*m_cigar)<<1 : 4;
			//fprintf(stderr,"allocing %ld\n", m_cigar);
...
static inline void ksw_backtrack(void *km, int is_rot, int is_rev, int with_N, const uint8_t *p, const int *off, const int *off_end, int n_col, 
								int i0, int j0,
...
static inline void ksw_reset_extz(ksw_extz_t *ez)
{
	...
	ez->n_cigar = 0, ez->m_cigar = 0, ez->zdropped = 0;
	ez->cigar = 0;
}
