...
def compare_cores(main_core_file, uf_cores, out_file, sd_file):
    ...
    for k in d:
        ...
        for interval in d[k]:
            ...
            overlaps = uf_int.overlap(
                interval.begin, interval.end
            )  # uf_int[interval.begin, interval.end]
    ...
    f = open(out_file, "w")
    f.write(
        f"{all_covered}\t{all_}\t{main_counter2}\t{main_counter}\t{len(d)}\t{all_covered / all_}\t{main_counter2/len(d)}\t{core_containers}\t{contains_sd}"
    )
