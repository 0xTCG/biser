...
def get_complex_elems(
    min_core_len=100,
    max_core_len=2000,
    out="align/results/test_elems/elems_test.fa",
    out_elems_path="align/results/test_elems/elems_test_rez.bed",
):
    list_of_elems = [
        get_random_sequence(random.randint(min_core_len, max_core_len)) for i in range(0, 5)
    ]
    ...
    # 2nd elem
    out_elems.write(
        f"chrA\t{len(list_of_elems[0])}\t{len(list_of_elems[0]) + len(list_of_elems[1])}\t1\t{len(list_of_elems[1])}\t+\n"
    )
    out_elems.write(
        f"chrB\t{len(list_of_elems[0])}\t{len(list_of_elems[0]) + len(list_of_elems[1])}\t1\t{len(list_of_elems[1])}\t+\n"
    )
    ...
    out_elems.write(f"chrA\t{s1}\t{e1}\t2\t{len(list_of_elems[2])}\t+\n")
    out_elems.write(
        f"chrC\t{len(list_of_elems[1])}\t{len(list_of_elems[1]) + len(list_of_elems[2])}\t2\t{len(list_of_elems[2])}\t+\n"
    )
...
def copy(initial_sequence, s1_begin, s1_end, s1_mate_begin):
    ...
    return (
        initial_sequence[:s1_mate_begin]
        + s_hold
        + initial_sequence[s1_mate_begin:]
        + get_random_sequence(10000),
        len(s_hold),
        updates,
    )
...
def one_iteration(
    ...
    while (
        len(
            sd_tree.overlap(
                core_begin - max_core_len - max_sd_len - min_sd_len,
                core_begin + max_core_len + max_sd_len + min_sd_len,
            )
        )
        > 0
    ...
    sd_len = random.randint(
        max(min_sd_len, core_len + min_core_len), core_len + min_core_len + max_sd_len
    )
    s1_begin = core_begin
    ...
    if (
        sd_tree.overlaps(s1_begin, s1_end)
        or sd_tree.overlaps(s1_mate_begin, s1_mate_end)
        or sd_tree.overlaps(s2_begin, s2_end)
        or sd_tree.overlaps(s2_mate_begin, s2_mate_end)
    ):
        print(s1_begin, s1_end)
        print(sd_tree)
        print(
            sd_tree.overlaps(s1_begin, s1_end),
            sd_tree.overlaps(s1_mate_begin, s1_mate_end),
            sd_tree.overlaps(s2_begin, s2_end),
            sd_tree.overlaps(s2_mate_begin, s2_mate_end),
        )
        assert False
    ...
    sd_coordinates[generation] = [
        ((s1_begin, s1_end), (s1_mate_begin, s1_mate_end)),
        ((s2_begin, s2_end), (s2_mate_begin, s2_mate_end)),
    ]
...
def main_funct_2(
    main_len, num_of_generations, seed, out_folder="test/simulation/", max_error=0.25
):
    random.seed(seed)
    global delta_main
    delta_main = 0.0
    ...
    write_file = open(
        f"{out_folder}simulations#{max_error_input}#{num_of_generations}#{seed}.fa", "w"
    )
    ...
    for i in range(0, num_of_generations):
        initial_sequence, main_len = one_iteration(
            sd_tree,
            initial_sequence,
            sd_coordinates,
            core_coordinates,
...
def calculate_coverages(path1, path2, threshold, output):
    ...
    main_d = (
        dict()
    )  # this is dict that has error as a key and number of successful coverages as value
    count = 0
